...
// Import local assets via Vite so they bundle correctly
import PhantomLogo from "@/Images/Phantom.png";
import SlushLogo from "@/Images/Slush.png";

interface WalletOption {
  id: string;
  name: string;
  icon: string;        // can be external URL or imported asset
  description?: string;
  circle?: boolean;    // render icon in a circle if true
}
...
export function ConnectWalletPage() {
  ...
  /** Renders the wallet icon with optional circle styling */
  const renderIcon = (wallet: WalletOption) => {
    if (wallet.circle) {
      // Circle wrapper
      return (
        <div className="w-10 h-10 rounded-full overflow-hidden bg-white/10 ring-1 ring-white/20 flex items-center justify-center">
          <img
            src={wallet.icon}
            alt={wallet.name}
            className="w-full h-full object-cover" // fills the circle
            onError={(e) => {
              // Fallback: show an initial if image fails to load
              const target = e.target as HTMLImageElement;
              target.style.display = "none";
              const parent = target.parentElement;
              if (parent) {
                parent.innerHTML = `<span class="text-white/80 text-sm font-semibold">${wallet.name.charAt(
                  0
                )}</span>`;
              }
            }}
